...
%       cada neurônio
        for nPeso = 1:model(1)
            for nAmostra = 1:C
                for nCoordenada = 1 : L-1
                    diferencas(nCoordenada, nAmostra) = ( w{1,1}(nPeso, nCoordenada) - x(nCoordenada + 1, nAmostra) ).^2;
                end
...
%       Para cada peso, pega as amostras pertencentes ao seu grupo,
        ...
        for nPeso_3 = 1:model(1)
            for nAmostra_3 = 1:C
                if( grupos(1, nAmostra_3) == nPeso_3 ) % se o elemento do grupo for igual ao número do peso:
                    soma{nPeso_3, 1}(:, nAmostra_3) = x(2:L, nAmostra_3); % armazena em soma todos os elementos pertencentes ao grupo do peso nPeso_3
    ...
    for nPeso_4 = 1 : model(1)
        for nAmostra_4 = 1:C
              if( grupos(1, nAmostra_4) == nPeso_4 ) % se o elemento do grupo for igual ao número do peso:
                  ...
                  difAMC(:, nElemen{nPeso_4,1}) = ( x(2:L, nAmostra_4) - w{1,1}(nPeso_4, :)' ).^2;
              end
        end
          som = sum(difAMC);
          som2 = sum(som);

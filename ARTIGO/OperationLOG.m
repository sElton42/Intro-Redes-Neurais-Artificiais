...
% uso do algoritmo de treinamento PMC com Função Logística
tic
[w, w_iniciais, epocas, EMQ, accuracyTrain] = BackPropOnPMC(zTreino, dTreino, eta, sigma, maxEpcs, model);
tempoGasto = toc
%% Aplicando o PMC com Função Logística
...
%% CHECAGEM DE ACERTOS DO TREINAMENTO COM F. LOGÍSTICA (1 neurônio)
% acertosTeste = 0;
% for nAmos = 1:numel(dTeste)
%     if( round(y{numel(model),1}(1,nAmos)) == dTeste(1,nAmos) ), acertosTeste = acertosTeste + 1; end
% end
% acertosTeste
% acertosTeste / numel(dTeste) * 100

%% CHECAGEM DE ACERTOS DO TREINAMENTO COM F. LOGÍSTICA (2 neurônios)
acertosTeste = 0;
...
for nAmos = 1 : size(dTeste,2)
    if( ( saida(1,nAmos) == dTeste(1,nAmos) ) & ( saida(2,nAmos) == dTeste(2,nAmos) ) )

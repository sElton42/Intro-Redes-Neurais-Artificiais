function [w, w_0, epcs, EMQ, accuracyTrain] = BackPropOnPMCTANH(x, d, eta, sigma, maxEpcs, model)
                ...
                if( numLayer == numel(model) )
                    for numLayerBack = numLayer : -1 : 1
                        ...
                        if(numLayerBack == numel(model))
                            ...
                            gLinha = ( 2 * exp( u{numLayerBack, 1}(:,numAmostra) ) ...
                            ...
                            if(numLayerBack == 1)
                                ...
                                for j = 1:size(w{numLayerBack, 1},1)
                                    gLinha = ( 2 * exp( u{numLayerBack, 1}(j,numAmostra) ) ...
    ...
    accuracyTrain = 0;
    for nAmos = 1:C
        if( (d(1,nAmos) >= 0 & y{numel(model),1}(1,nAmos) >= 0) | (d(1,nAmos) < 0 & y{numel(model),1}(1,nAmos) < 0) ), accuracyTrain = accuracyTrain + 1; end
    end
    accuracyTrain
end